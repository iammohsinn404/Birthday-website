...
let dpr = 1, particles = [], rafRunning = false;
...
function confettiBurst(cx, cy, count){
  ...
  const colors = ['#ff4e91','#ff9dc0','#8b5cf6','#ffc861','#7c4df0','#ffffff'];
  ...
  if(!rafRunning){ rafRunning = true; requestAnimationFrame(tickConfetti); }
}
function tickConfetti(){
  ...
  for(let i=particles.length-1; i>=0; i--){
    ...
    if(p.life >= p.maxLife || p.y > innerHeight + 40){ particles.splice(i,1); continue; }
    ctx.save();
  ...
  else { rafRunning = false; ctx.clearRect(0,0,fxCanvas.width,fxCanvas.height); }
}
...
yes?.addEventListener('click', () => {
  ...
  confettiBurst(innerWidth/2, innerHeight*0.32, 130);
});

let noCount = 0;
const noTexts = ["NO! 😐","Are you sure? 😭","Really? 😤","Come onnn! 🥺","PLEASE! 😂","Okay okay... 😭"];
no?.addEventListener('click', () => {
  noCount++;
  no.textContent = noTexts[Math.min(noCount, noTexts.length-1)];
  no.style.transform = `translate(${Math.random()*120-60}px,${Math.random()*70-35}px)`;
  if(noCount >= 5) no.style.display = 'none';
});

/* shared "reveal the surprise" mechanism, triggered from the hero button or the nav pill */
function triggerSurprise(originEl){
  surprise.classList.add('show');
  surprise.scrollIntoView({behavior:'smooth', block:'center'});
  const r = (originEl || surpriseBtn).getBoundingClientRect();
  confettiBurst(r.left + r.width/2, r.top + r.height/2, 150);
  document.body.classList.add('shake');
  setTimeout(() => document.body.classList.remove('shake'), 500);
}
surpriseBtn?.addEventListener('click', () => triggerSurprise(surpriseBtn));
navCta?.addEventListener('click', () => triggerSurprise(navCta));

replayBtn?.addEventListener('click', () => {
  const r = replayBtn.getBoundingClientRect();
  confettiBurst(r.left + r.width/2, r.top + r.height/2, 160);
  confettiBurst(innerWidth*0.15, innerHeight*0.5, 60);
  confettiBurst(innerWidth*0.85, innerHeight*0.5, 60);
});
...
const navTargets = [...document.querySelectorAll('.section-dots .dot, .nav-links a')];
const dotSectionIds = ['heroSection','messageSection','storySection','memoriesSection','surpriseSection'];
const dotSections = dotSectionIds.map(id => document.getElementById(id)).filter(Boolean);
  ...
  const storyIO = new IntersectionObserver((entries) => {
    entries.forEach(en => {
      if(en.isIntersecting){
        ...
        if(storyRailFill) storyRailFill.style.height = ((idx+1)/storyItems.length*100) + '%';
      }
    });
  }, {threshold:.5, rootMargin:'-10% 0px -10% 0px'});
...
if(cinematicLines){
  const cineIO = new IntersectionObserver((entries) => {
    entries.forEach(en => {
      if(en.isIntersecting){
...
if(!prefersReducedMotion){
  ...
  for(let i=0;i<16;i++){
    ...
    s.style.color = Math.random() > .5 ? 'rgba(255,94,155,.4)' : 'rgba(139,92,246,.35)';
...
function updateReel(){
  ...
  reelFrames.forEach((f,i) => {
    ...
    let opacity, ty, scale, z, active = false;
    if(t <= 0){ opacity = 0; ty = 70; scale = .86; z = 1; }
    else if(t < 1){ const e = easeOutCubic(t); opacity = e; ty = 70*(1-e); scale = .86 + .14*e; z = 3; active = true; }
    else if(t < 2){ const e = easeOutCubic(t-1); opacity = 1-e; ty = -80*e; scale = 1-.08*e; z = 2; }
    else { opacity = 0; ty = -80; scale = .92; z = 1; }
...
function onScroll(){
  ...
  if(!scrollTicking){
    requestAnimationFrame(() => { updateReel(); scrollTicking = false; });
    scrollTicking = true;
  }
}
...
const photosToLoad = ['images/decoration.jpg','images/cake-cat.jpg','images/happybirthday-card.jpg'];
let loadedCount = 0;
function bumpPreloader(){
  loadedCount++;
  preloaderFill.style.width = Math.round((loadedCount/photosToLoad.length)*100) + '%';
}
const loadPromises = photosToLoad.map(src => new Promise(resolve => {
  const im = new Image();
  im.onload = im.onerror = () => { bumpPreloader(); resolve(); };
  im.src = src;
}));
